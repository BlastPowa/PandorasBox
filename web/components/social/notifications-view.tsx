...
export function NotificationsView() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex max-w-full gap-2 overflow-x-auto pb-1">{FILTERS.map((item) => <button key={item.value} onClick={() => setFilter(item.value)} className={`min-h-11 shrink-0 rounded-full border px-4 text-sm font-semibold transition ${filter === item.value ? "border-transparent bg-[var(--accent)] text-black shadow-[0_10px_24px_rgb(var(--accent-rgb)/0.22)]" : "border-[var(--border)] bg-[var(--glass)] text-[var(--text-secondary)] hover:-translate-y-0.5 hover:border-[rgb(var(--accent-rgb)/0.35)]"}`}>{item.label}</button>)}</div>
      <Button variant="ghost" size="sm" onClick={() => void update("read_all")}><CheckCheck className="size-4" /> Mark all read</Button>
    </div>
    {loading ? <div className="skeleton h-36 rounded-[var(--radius-lg)]" /> : rows.length === 0 ? <div className="pb-uiverse-card pb-uiverse-card--feature grid min-h-48 place-items-center rounded-[var(--radius-lg)] text-center"><div><Bell className="mx-auto mb-2 size-9 text-[var(--accent)]" /><h2 className="font-display font-bold">All caught up</h2><p className="mt-1 text-sm text-[var(--text-muted)]">Social notifications will appear here.</p></div></div> : <div className="space-y-2">{rows.map((row) => {
      const content = copy(row); const unread = !row.read_at;
      return <article key={row.id} className={`pb-uiverse-row flex items-start gap-3 rounded-[var(--radius-lg)] p-4 ${unread ? "ring-1 ring-[rgb(var(--accent-rgb)/0.45)]" : ""}`}>
        <span className="pb-uiverse-icon grid size-11 shrink-0 place-items-center rounded-full text-[var(--accent)]">{row.type === "share_received" ? <Share2 className="size-5" /> : row.type === "message_received" || row.type === "group_invitation" ? <Bell className="size-5" /> : <UserPlus className="size-5" />}</span>
