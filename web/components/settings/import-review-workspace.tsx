...
export function ImportReviewWorkspace({
  ...
  return (
    <Dialog.Root open onOpenChange={(open) => { if (!open) onClose(); }}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-[99] bg-slate-900/20 backdrop-blur-[2px]" />
        <Dialog.Content className="fixed inset-0 z-[100] flex outline-none md:items-center md:justify-center md:p-5">
      <section className="flex h-full min-w-0 w-full flex-col overflow-hidden bg-[var(--bg-surface)] md:h-[min(90vh,900px)] md:max-w-6xl md:rounded-2xl md:border md:border-[var(--border-strong)] md:shadow-[0_24px_80px_rgba(15,23,42,0.18)]">
        <header className="sticky top-0 z-20 border-b border-[var(--border)] bg-[var(--bg-surface)] px-4 pb-3 pt-[max(1rem,var(--safe-top))] md:px-6">
              ...
              <article key={row.id} className={cn("rounded-2xl border p-3 md:p-4", row.resolutionState === "review" || row.resolutionState === "failed" ? "border-amber-400/40 bg-amber-400/5" : "border-[var(--border)] bg-[var(--bg-surface)]")}>
                <div className="flex min-w-0 gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {row.duplicateOf && <p className="mt-1 text-xs text-amber-700">Duplicate input — kept for review.</p>}
        ...
        <footer className="sticky bottom-0 z-20 border-t border-[var(--border)] bg-[var(--bg-surface)] px-4 pb-[max(1rem,var(--safe-bottom))] pt-3 md:px-6">
      ...
      {filtersOpen && <div className="fixed inset-0 z-[110] flex items-end bg-slate-900/15 backdrop-blur-[2px] md:items-center md:justify-center" onMouseDown={(event) => { if (event.currentTarget === event.target) setFiltersOpen(false); }}><section role="dialog" aria-modal="true" aria-label="Import filters" className="max-h-[100dvh] w-full overflow-y-auto rounded-t-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-[0_24px_80px_rgba(15,23,42,0.18)] md:max-w-md md:rounded-2xl">
      ...
      {editing && <div className="fixed inset-0 z-[120] flex bg-slate-900/20 backdrop-blur-[2px] md:items-center md:justify-center md:p-5"><section role="dialog" aria-modal="true" aria-label="Change title match" className="flex h-full w-full flex-col overflow-hidden bg-[var(--bg-surface)] md:h-[min(86vh,820px)] md:max-w-3xl md:rounded-2xl md:border md:border-[var(--border)] md:shadow-[0_24px_80px_rgba(15,23,42,0.18)]">
        <header className="sticky top-0 z-10 border-b border-[var(--border)] bg-[var(--bg-surface)] p-4 pt-[max(1rem,env(safe-area-inset-top))] md:p-5"><div className="flex items-center justify-between"><h3 className="text-lg font-bold">Change match</h3><button type="button" className="grid size-11 place-items-center rounded-full text-[var(--text-secondary)] transition hover:bg-[var(--bg-elevated)] hover:text-[var(--text)]" onClick={() => setEditingId(null)} aria-label="Close candidate search"><X /></button></div>
