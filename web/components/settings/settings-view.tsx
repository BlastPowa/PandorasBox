                ...
                <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)]">
                  ...
                  <div className="max-h-[32rem] space-y-2 overflow-y-auto p-3">
                    {preparedRows.map((row, index) => (
                      <div key={row.id} className="grid grid-cols-[2rem_minmax(0,1fr)_2.75rem] items-start gap-2 rounded-xl border border-[var(--border)] p-2 sm:grid-cols-[2rem_minmax(0,1fr)_5.5rem_2.75rem]">
                        <span className="col-start-1 row-start-1 pt-3 text-center font-mono text-xs text-[var(--text-muted)]">{index + 1}</span>
                        <label className="col-start-2 row-start-1 min-w-0"><span className="sr-only">Title {index + 1}</span><input value={row.title} onChange={(event) => setPreparedRows((current) => markDuplicateRows(current.map((entry) => entry.id === row.id ? { ...entry, title: event.target.value } : entry)))} className="min-h-11 min-w-0 w-full rounded-lg border border-[var(--border)] bg-[var(--bg-base)] px-3 text-sm" />{row.duplicateOf && <span className="mt-1 block text-[11px] text-amber-300">Duplicate row</span>}</label>
                        <label className="col-start-2 row-start-2 min-w-0 sm:col-start-3 sm:row-start-1"><span className="sr-only">Year {index + 1}</span><input inputMode="numeric" value={row.year ?? ""} placeholder="Year" onChange={(event) => setPreparedRows((current) => markDuplicateRows(current.map((entry) => entry.id === row.id ? { ...entry, year: Number(event.target.value) || null } : entry)))} className="min-h-11 min-w-0 w-full rounded-lg border border-[var(--border)] bg-[var(--bg-base)] px-2 text-sm" /></label>
                        <button type="button" className="col-start-3 row-start-1 grid size-11 place-items-center rounded-lg hover:bg-[var(--glass)] sm:col-start-4" aria-label={`Remove ${row.title}`} onClick={() => setPreparedRows((current) => current.filter((entry) => entry.id !== row.id))}><Trash2 className="size-4" /></button>
                      </div>
                    ))}
                  </div>
