...
/* Provider marks are tiny third-party service logos; keep them as plain images. */
...
export function DiscoverGrid({ kind, initial }: Props) {
  ...
  function surpriseMe() {
    ...
    router.push(mediaItemHref(pick));
  }
  ...
  return (
    <div className="space-y-6">
      <DiscoveryPageHeader title={kind === "movie" ? "Movies" : "TV Shows"} description={`Discover ${kind === "movie" ? "films" : "series"} by genre, year, popularity, and streaming provider.`} actions={<div className="flex flex-wrap items-center gap-2">
          ...
          <Dialog.Root open={filtersOpen} onOpenChange={handleFiltersOpen}><Dialog.Trigger asChild><button type="button" className="inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-full bg-[var(--accent)] px-5 text-xs font-bold text-white shadow-sm transition hover:bg-[var(--accent-hover)] md:hidden"><SlidersHorizontal className="size-4" /> Filters{hasFilters ? " · Active" : ""}</button></Dialog.Trigger><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-50 bg-[var(--bg-base)]/35 backdrop-blur-[3px]" /><Dialog.Content className="fixed inset-x-0 bottom-0 z-50 max-h-[90dvh] overflow-y-auto rounded-t-[28px] border-t border-[var(--border)] bg-[color-mix(in_srgb,var(--bg-surface)_94%,transparent)] p-5 pb-[calc(20px+env(safe-area-inset-bottom))] shadow-2xl backdrop-blur-xl"><div className="mb-5 flex items-center justify-between"><Dialog.Title className="font-display text-xl font-bold">Filters</Dialog.Title><div className="flex items-center gap-3"><button type="button" onClick={() => { setDraftGenres([]); setDraftYear(""); setDraftProvider(""); setDraftSort("popular"); }} className="text-xs font-semibold text-[var(--text-muted)]">Clear all</button><Dialog.Close className="grid size-9 place-items-center rounded-full border border-[var(--border)] bg-[var(--bg-elevated)] text-[var(--text-secondary)] transition hover:text-[var(--text)]"><X className="size-4" /></Dialog.Close></div></div><section className="space-y-3"><p className="text-xs font-bold text-[var(--text-secondary)]">Content</p><span className="inline-flex rounded-full border border-[rgb(var(--accent-rgb)/0.28)] bg-[rgb(var(--accent-rgb)/0.10)] px-4 py-2 text-xs font-bold text-[var(--accent)]">{kind === "movie" ? "Movies" : "TV Shows"}</span></section><section className="mt-5 space-y-3"><p className="text-xs font-bold text-[var(--text-secondary)]">Genres</p><div className="flex flex-wrap gap-2">{genreOptions.map((option) => { const active = draftGenres.includes(option.value); return <button key={option.value} type="button" aria-pressed={active} onClick={() => setDraftGenres((current) => active ? current.filter((value) => value !== option.value) : [...current, option.value])} className={`rounded-full px-3 py-2 text-xs font-semibold ${active ? "bg-[var(--accent)] text-white shadow-sm" : "border border-[var(--border)] bg-[var(--bg-elevated)] text-[var(--text-secondary)]"}`}>{option.label}</button>; })}</div></section><div className="mt-5 grid gap-4"><label className="space-y-2 text-xs font-bold text-[var(--text-secondary)]">Sort by<select value={draftSort} onChange={(event) => setDraftSort(event.target.value as SortKey)} className="block h-12 w-full rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)] px-4 text-sm text-[var(--text)]">{SORT_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="space-y-2 text-xs font-bold text-[var(--text-secondary)]">Year<select value={draftYear} onChange={(event) => setDraftYear(event.target.value)} className="block h-12 w-full rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)] px-4 text-sm text-[var(--text)]"><option value="">All years</option>{yearOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div><section className="mt-5 space-y-3"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold text-[var(--text-secondary)]">Provider</p>{draftProvider && <button type="button" onClick={() => setDraftProvider("")} className="text-[11px] font-semibold text-[var(--accent)]">All providers</button>}</div><div className="-mx-1 flex snap-x gap-2 overflow-x-auto px-1 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><button type="button" aria-pressed={!draftProvider} onClick={() => setDraftProvider("")} className={`min-h-12 shrink-0 snap-start rounded-xl border px-3 text-xs font-bold ${!draftProvider ? "border-[var(--accent)] bg-[rgb(var(--accent-rgb)/0.10)] text-[var(--text)]" : "border-[var(--border)] bg-[var(--bg-elevated)] text-[var(--text-secondary)]"}`}>All</button>{STREAMING_PROVIDERS.map((streamingProvider) => { const active = draftProvider === streamingProvider.slug; return <button key={streamingProvider.slug} type="button" aria-pressed={active} onClick={() => setDraftProvider(streamingProvider.slug)} className={`flex min-h-12 shrink-0 snap-start items-center gap-2 rounded-xl border px-2.5 py-2 text-xs font-semibold ${active ? "border-[var(--accent)] bg-[rgb(var(--accent-rgb)/0.10)] text-[var(--text)] shadow-sm" : "border-[var(--border)] bg-[var(--bg-elevated)] text-[var(--text-secondary)]"}`}>{ }<img src={providerLogoUrl(streamingProvider)} alt="" aria-hidden="true" className="size-7 rounded-lg object-cover" /><span className="max-w-24 truncate">{streamingProvider.name}</span></button>; })}</div></section><button type="button" onClick={applyMobileFilters} className="mt-6 h-12 w-full rounded-xl bg-[var(--accent)] text-sm font-bold text-white shadow-sm transition hover:bg-[var(--accent-hover)]">Show results</button></Dialog.Content></Dialog.Portal></Dialog.Root>
