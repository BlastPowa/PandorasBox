...
export function HomeHeroActions({ item, href }: { item: UnifiedSearchResult; href: string }) {
  ...
  return (
    <div className="mt-1 flex flex-wrap items-center gap-3">
      {watchHref && <Link href={watchHref} className="inline-flex h-12 items-center gap-2 rounded-full bg-white px-5 text-xs font-black uppercase tracking-wide text-black shadow-[0_12px_34px_rgba(0,0,0,0.3)] transition hover:-translate-y-0.5 hover:bg-white/90 sm:px-7 sm:text-sm"><Play className="size-4 fill-current" /> Watch now</Link>}
      <Link href={href} className="glass inline-flex h-12 items-center gap-2 rounded-full px-5 text-xs font-bold uppercase tracking-wide transition hover:border-[var(--accent)] hover:text-[var(--accent)] sm:px-6 sm:text-sm">View details <ArrowRight className="size-4" /></Link>
