...
export function SeasonPicker({ season, year }: { season: AnimeSeason; year: number }) {
  ...
  return (
    <div className={cn("flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3", pending && "opacity-60")}>
      <div className="-mx-1 flex overflow-x-auto rounded-full border border-[var(--border)] bg-[var(--bg-base)] p-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0">
        {ANIME_SEASONS.map((s) => (
          <button
            key={s.value}
      ...
      <select
        ...
        className="min-h-10 w-full rounded-full border border-[var(--border)] bg-[var(--bg-base)] px-4 py-2 text-xs font-semibold text-[var(--text)] outline-none focus:border-[var(--accent)] sm:w-auto"
      >
        {seasonYears().map((y) => (
