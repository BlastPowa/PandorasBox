...
export async function GET(
  ...
  if (!providerIsConfigured(cfg) || !clientId)
    return NextResponse.json(
      {
        error: cfg.id === "trakt" || cfg.id === "simkl"
          ? `${cfg.name} is not configured. Set ${cfg.id.toUpperCase()}_CLIENT_ID and ${cfg.id.toUpperCase()}_CLIENT_SECRET in the environment.`
          : `${cfg.name} is not configured. Set ${cfg.id.toUpperCase()}_CLIENT_ID in the environment.`,
      },
      { status: 503 }
    );
